package br.com.rafaelvieira.task_management.unit.service;
...
import br.com.rafaelvieira.task_management.service.impl.TaskServiceImpl;
...
/**
 * Testes Unitários utilizando JUnit 6
 * Demonstrando novas funcionalidades do JUnit 6
 */
...
class TaskServiceTest {
    ...
    void shouldCreateTaskWithCategory() {
        ...
        assertThat(result)
                .isNotNull()
                .satisfies(t -> {
                    assertThat(t.title()).isEqualTo("Work Task");
                    assertThat(t.categoryName()).isEqualTo("Work");
                });
    ...
    void shouldFindTasksByAllStatuses(TaskStatus status) {
        ...
        assertThat(result)
                .isNotEmpty()
                .hasSize(1)
                .allSatisfy(t -> assertThat(t.status()).isEqualTo(status));
    ...
    void shouldFindTasksByAllPriorities(Priority priority) {
        Task task = Task.builder()
                .id(1L)
                .title("Task")
                .status(TaskStatus.TODO)
    ...
    void shouldGetAllTasks() {
        List<Task> tasks = Arrays.asList(
                Task.builder().id(1L).title("Task 1").status(TaskStatus.TODO).priority(Priority.LOW).build(),
                Task.builder().id(2L).title("Task 2").status(TaskStatus.IN_PROGRESS).priority(Priority.MEDIUM).build(),
                Task.builder().id(3L).title("Task 3").status(TaskStatus.DONE).priority(Priority.HIGH).build()
        );
    ...
    void shouldGetTasksByUserId() {
        ...
        List<Task> tasks = Arrays.asList(
                Task.builder().id(1L).title("Task 1").assignedUser(user).status(TaskStatus.TODO).priority(Priority.MEDIUM).build(),
                Task.builder().id(2L).title("Task 2").assignedUser(user).status(TaskStatus.TODO).priority(Priority.MEDIUM).build()
        );
        ...
        assertThat(result)
                .hasSize(2)
                .allSatisfy(t -> assertThat(t.assignedUserName()).isEqualTo("testuser"));
    ...
    void shouldFindOverdueTasks() {
        ...
        assertThat(result)
                .hasSize(2)
                .allSatisfy(t -> assertThat(t.overdue()).isTrue());
    ...
    class TaskWithUserTests {
        ...
        void shouldCreateTaskWithAssignedUser() {
            ...
            assertThat(result)
                    .isNotNull()
                    .satisfies(t -> {
                        assertThat(t.assignedUserName()).isEqualTo("developer");
                        assertThat(t.title()).isEqualTo("Development Task");
                    });
