package br.com.rafaelvieira.taskmanagement.integration.repository;
...
/**
 * Testes de Integração do Repositório usando Testcontainers Demonstrando integração real com
 * Postgres
 *
...
class TaskRepositoryIntegrationTest extends BaseIntegrationTest {
    ...
    void setUp() {
        ...
        savedCategory =
                categoryRepository.save(
                        Category.builder().name("Work").description("Work related tasks").build());
    ...
    void testShouldSaveAndRetrieveTask() {
        ...
        assertThat(retrieved)
                ...
                .satisfies(
                        t -> {
                            assertThat(t.getTitle())
                                    .as("título da tarefa")
                                    .isEqualTo("Integration Test Task");
                            assertThat(t.getDescription())
                                    .as("descrição da tarefa")
                                    .isEqualTo("Testing database integration");
                            assertThat(t.getStatus())
                                    .as("status da tarefa")
                                    .isEqualTo(TaskStatus.TODO);
                            assertThat(t.getPriority())
                                    .as("prioridade da tarefa")
                                    .isEqualTo(Priority.HIGH);
                            assertThat(t.getCategory().getName())
                                    .as("categoria da tarefa")
                                    .isEqualTo("Work");
                            assertThat(t.getAssignedUser().getUsername())
                                    .as("usuário atribuído à tarefa")
                                    .isEqualTo("testUser");
                            assertThat(t.getCreatedAt())
                                    .as("timestamp de criação da tarefa")
                                    .isNotNull();
                            assertThat(t.getUpdatedAt())
                                    .as("timestamp de atualização da tarefa")
                                    .isNotNull();
                        });
    }
                ...
                .allSatisfy(
                        task -> {
                            assertThat(task.getDueDate())
                                    .as("data de vencimento da tarefa")
                                    .isBefore(LocalDateTime.now());
                            assertThat(task.getStatus())
                                    .as("status da tarefa")
                                    .isNotEqualTo(TaskStatus.DONE);
                        });
    }
    ...
    void testShouldFindTasksByUserAndStatus() {
        ...
        List<Task> userTodoTasks =
                taskRepository.findByUserIdAndStatus(savedUser.getId(), TaskStatus.TODO);
        List<Task> userDoneTasks =
                taskRepository.findByUserIdAndStatus(savedUser.getId(), TaskStatus.DONE);
    ...
    void testShouldUpdateTaskAndVerifyTimestamp() {
        Task task =
                taskRepository.save(
                        Task.builder()
        ...
        assertThat(updatedTask.getTitle())
                .as("título atualizado da tarefa")
                .isEqualTo("Updated Title");
        assertThat(updatedTask.getUpdatedAt())
                .as("timestamp de atualização da tarefa")
                .isAfter(originalUpdatedAt);
        assertThat(updatedTask.getCreatedAt())
                .as("timestamp de criação da tarefa permanece inalterado")
