...
public interface TaskRepository extends JpaRepository<@NotNull Task, @NotNull Long> {
        ...
        @Query("SELECT COUNT(t) FROM Task t WHERE t.squad.id = :squadId AND t.dueDate < :now AND"
        ...
        @Query("SELECT t FROM Task t WHERE t.squad.id IN "
                        + "(SELECT sm.squad.id FROM SquadMember sm WHERE sm.user = :user)")
        List<Task> findSquadTasksForMember(@Param("user") User user);

        @Query("SELECT DISTINCT t FROM Task t WHERE t.createdBy = :user OR t.assignedUser = :user OR"
        ...
        @Query("SELECT t FROM Task t WHERE t.squad.id = :squadId AND t.mainStartedAt IS NOT NULL AND"
        ...
        @Query("SELECT COUNT(t) FROM Task t WHERE t.status = :status AND ((t.assignedUser = :user AND t.squad IS NULL) OR t.squad.lead = :user)")
        long countByStatusForLead(@Param("status") TaskStatus status, @Param("user") User user);

        @Query("SELECT COUNT(DISTINCT t) FROM Task t WHERE t.status = :status AND (t.createdBy = :user OR t.assignedUser = :user OR t.squad.id IN (SELECT sm.squad.id FROM SquadMember sm WHERE sm.user = :user))")
        long countByStatusForMember(@Param("status") TaskStatus status, @Param("user") User user);

        @Query("SELECT t FROM Task t WHERE t.status = :status AND ((t.assignedUser = :user AND t.squad IS NULL) OR t.squad.lead = :user)")
        List<Task> findByStatusForLead(@Param("status") TaskStatus status, @Param("user") User user);

        @Query("SELECT DISTINCT t FROM Task t WHERE t.status = :status AND (t.createdBy = :user OR t.assignedUser = :user OR t.squad.id IN (SELECT sm.squad.id FROM SquadMember sm WHERE sm.user = :user))")
        List<Task> findByStatusForMember(@Param("status") TaskStatus status, @Param("user") User user);

        @Query("SELECT t FROM Task t WHERE t.dueDate < :date AND t.status != 'DONE' AND ((t.assignedUser = :user AND t.squad IS NULL) OR t.squad.lead = :user)")
        List<Task> findOverdueTasksForLead(@Param("date") LocalDateTime date, @Param("user") User user);

        @Query("SELECT DISTINCT t FROM Task t WHERE t.dueDate < :date AND t.status != 'DONE' AND (t.createdBy = :user OR t.assignedUser = :user OR t.squad.id IN (SELECT sm.squad.id FROM SquadMember sm WHERE sm.user = :user))")
        List<Task> findOverdueTasksForMember(@Param("date") LocalDateTime date, @Param("user") User user);

        @Query("SELECT COUNT(t) FROM Task t WHERE t.priority = :priority AND ((t.assignedUser = :user AND t.squad IS NULL) OR t.squad.lead = :user)")
        long countByPriorityForLead(@Param("priority") Priority priority, @Param("user") User user);

        @Query("SELECT COUNT(DISTINCT t) FROM Task t WHERE t.priority = :priority AND (t.createdBy = :user OR t.assignedUser = :user OR t.squad.id IN (SELECT sm.squad.id FROM SquadMember sm WHERE sm.user = :user))")
