...
public interface TaskService {
    ...
    /**
     * Updates a task with role-based restrictions. MEMBERs cannot change dueDate on squad tasks.
     */
    ...
    /**
     * Returns tasks visible to the current user based on their role: - ADMIN: all tasks - LEAD:
     * personal tasks + tasks from squads they lead - MEMBER: personal tasks + tasks from squads
     * they belong to
     */
    ...
    /** Deletes a task with role-based restrictions. MEMBERs cannot delete squad tasks. */
    void deleteTask(Long id, User currentUser);

    TaskRecord changeTaskStatus(Long id, TaskStatus newStatus);

    /**
     * Changes task status with role-based restrictions. MEMBERs cannot control timers on squad
     * tasks.
     */
